...
client = Client("http://service-nrt.geonet.org.nz" , timeout=20)
#client = Client()
stime = UTCDateTime('2021-320T01:00:00')
#etime = UTCDateTime('2021-315T00:00:00')
etime = stime + 60*60
st = client.get_waveforms("NZ", "RIZ", "1*",
                           "HH*", stime, etime, attach_response = True)

st.merge()
st.sort()
st.trim(starttime=stime+5, endtime=etime-5)
print(st)
per, nlnm = get_nlnm()
per, nhnm = get_nhnm()
NFFT=4096
...
plt.subplot(1,1,1)
for idx, tr in enumerate(st.select(channel="HH*")):
    label='TC120-PH2 ' + (tr.id).replace('.',' ')
...
# plt.subplot(2,1,2)
# for idx, tr in enumerate(st2.select(channel="HH*")):
#     label='TC120-PH2 ' + (tr.id).replace('.',' ')
#     label = label.replace('HHZ', 'HHU')
#     label = label.replace('HH1', 'HHV')
#     label = label.replace('HH2', 'HHW')
#     f, p = signal.welch(tr.data, fs = tr.stats.sampling_rate,
#                      nperseg=NFFT, noverlap=256)
#     amp, f= tr.stats.response.get_evalresp_response(tr.stats.delta, NFFT,
#                                                     output='ACC')
...
plt.show()
# plt.savefig('WHSZ_psd_314_315_2021.png', format='PNG', dpi=400)