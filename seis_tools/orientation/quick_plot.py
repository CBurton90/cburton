...
t1 = UTCDateTime("2021-05-26T13:05:00")

# st = read("/home/conradb/seis_tools/orientation/NIC_BH_PH_mseed/T120-BH1/NIC_centaur-6_7978_20210518_000000.seed")
# inv = read_inventory("/home/conradb/seis_tools/orientation/NIC_BH_PH_mseed/T120-BH1/NZ.NIC.10.xml/NZ.NIC.10.HH*.xml")
# st = get_and_remove_response(stream=st, inventory=inv, output='VEL', t1=t1, duration=t1 + 60*5)
...
st_2 = get_and_remove_response(stream=st_2, inventory=inv2, output='VEL', t1=t1, duration=t1 + 60*5)
...
# 	Rad_rot =  np.cos(theta)*st.select(id="NZ.NIC.10.HH1") - np.sin(theta)*st.select(id="NZ.NIC.10.HH2")
# 	Trv_rot = np.sin(theta)*st.select(id="NZ.NIC.10.HH1") + np.cos(theta)*st.select(id="NZ.NIC.10.HH2")
# 	N_surface = st_2.select(id="NZ.SS01.10.HHN")
# 	# Ncorr = np.correlate(Rad_rot[0].data, N_surface[0].data, mode='full')
...
# Final =  np.cos(rotangle)*st.select(id="NZ.NIC.10.HH1") - np.sin(rotangle)*st.select(id="NZ.NIC.10.HH2")

# fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, sharex=False, figsize=(15,15))
# fig.suptitle('T120-BH1 Orientation NIC', fontsize=18)

# tr_1 = st.select(id="NZ.NIC.10.HH1")
# t = tr_1[0].times()
