...
def fold_ccfs(zz,nz,ez):
	...
	# Order ccf streams
	st_folded.sort() 
	ZZ = st_folded.select(id="NZ.MWZ.10.HHZ")
	RZ = st_folded.select(id="NZ.MWZ.10.HH1")
	TZ = st_folded.select(id="NZ.MWZ.10.HH2")

	return ZZ,RZ,TZ

	
ZZ,RZ,TZ = fold_ccfs(zz='MWZ_PUZ_ZZ.mseed',nz='MWZ_PUZ_1Z.mseed',ez='MWZ_PUZ_2Z.mseed')
...
back_azi = gps2dist_azimuth(MWZ_lat, MWZ_long, PUZ_lat, PUZ_long)
print(back_azi)
...
nylat = MWZ_lat; nylon = MWZ_long
# lonlat, lonlon are lat/lon of London.
lonlat = PUZ_lat; lonlon = PUZ_long
...
ax1.set_title('MWZ to PUZ', pad=10)
...
# fig.tight_layout()
plt.show()
# plt.savefig('T120-BH1_orient_MWZ_PUZ.png', format='PNG', dpi=400)
